    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride > 1 or in_planes != planes * Bottleneck.expansion:
            self.downsample = nn.Sequential(OrderedDict([
                ...
                ("0", nn.Conv2d(in_planes, planes * self.expansion, kernel_size=1, bias=False))
...
class AttentionPool2d(nn.Module):
    def __init__(self, special_dim: int, embed_dim: int, num_heads: int, output_dim: int = None):
        super(AttentionPool2d, self).__init__()
        self.positional_embedding = nn.Parameter(torch.randn(special_dim**2+1, embed_dim)/embed_dim**0.5)
    ...
    def forward(self, x):
        ...
        x += self.positional_embedding[:, None, :].to(x.dtype) # (HW+1)NC
...
class ModifiedResNet(nn.Module):
    ...
    - There are now 3 "stem" convolutions as opposed to 1, with avgpool instead of maxpool.
    ...
    def __init__(self, layers, output_dim, heads, input_res=224, width=64):
        ...
        embed_dim = width*8*4       # the ResNet feature dimension
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x:torch.Tensor):
        ...
        x += self.positional_embedding.to(x.dtype)*torch.arange(x.shape[1], device=x.device).unsqueeze(0)
        x = self.ln_pre(x)
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        x = self.token_embedding(text).type(self.dtype)         # shape = [batch_size, context_length, d_model]
        ...
        # take features from the eot embedding (eot embedding is the highest number in each sequence)
...
def convert_weights(model: nn.Module):
    """Convert applicable model params to fp16"""

    def _convert_weights_to_fp16():
        ...
        for name in ["text_projection", "proj"]:
            if hasattr(l, name):
                getattr(l, name).data = getattr(l, name).data.half()
